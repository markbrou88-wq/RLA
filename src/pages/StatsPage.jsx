// StatsPage.jsx
// Skaters: read from leaders_current so the table is populated
// Goalies: read from goalie_stats_current so SA/GA/W/L/OTL/SO follow edits.
...
export default function StatsPage() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: sk, error: skErr }, { data: gs, error: gsErr }] =
        await Promise.all([
          // SKATERS: use leaders_current (has player, team, gp, g, a, pts)
          supabase
            .from("leaders_current")
            .select("player_id, player, team, gp, g, a, pts")
            .order("pts", { ascending: false }),
          // GOALIES: aggregated goalie view
          supabase
            .from("goalie_stats_current")
            .select(
              "player_id, goalie, team, sa, ga, sv_pct, gaa, wins, losses, otl, so"
            )
      ...
      if (skErr) {
        console.error("Error loading skater stats:", skErr);
        setSkaters([]);
      } else {
        setSkaters(sk || []);
      }
      ...
      if (gsErr) {
        console.error("Error loading goalie stats:", gsErr);
        setGoalies([]);
      } else {
        setGoalies(gs || []);
      }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <button
          className={`btn ${tab === "skaters" ? "btn-blue" : "btn-grey"}`}
          onClick={() => setTab("skaters")}
        >
          Skaters
        </button>
        <button
          className={`btn ${tab === "goalies" ? "btn-blue" : "btn-grey"}`}
            ...
            <tbody>
              {skaters.map((r) => (
                <tr key={r.player_id} style={rowS}>
                  <td style={tdS}>
                    <Link to={`/players/${r.player_id}`}>{r.player}</Link>
                  </td>
                  <td style={tdS}>{r.team}</td>
                  <td style={tdR}>{r.gp}</td>
                  <td style={tdR}>{r.g}</td>
                  <td style={tdR}>{r.a}</td>
                  <td style={{ ...tdR, fontWeight: 600 }}>{r.pts}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {goalies.map((r) => (
                <tr key={r.player_id} style={rowS}>
                  <td style={tdS}>
                    <Link to={`/players/${r.player_id}`}>{r.goalie}</Link>
                  </td>
                  <td style={tdS}>{r.team}</td>
