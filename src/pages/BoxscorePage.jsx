  ...
  React.useEffect(()=>{
    (async()=>{
      ...
      const [grH,grA] = await Promise.all([
        supabase.from("game_rosters").select("players:player_id(id,name,number,position)").eq("game_id",g.id).eq("team_id",g.home_team_id).eq("dressed",true),
        supabase.from("game_rosters").select("players:player_id(id,name,number,position)").eq("game_id",g.id).eq("team_id",g.away_team_id).eq("dressed",true)
      ]);
        ...
        <>
          ...
          <div className="card" style={{marginTop:16}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>
                {rows.length===0 && <tr><td colSpan={5} style={{padding:12,color:"#888"}}>—</td></tr>}
                {rows.map((r,i)=>{
                  if(r.goal){
                    const teamShort=r.goal.teams?.short_name||"";
                    const main=r.goal.players?.name||(r.goal.players?.number?`#${r.goal.players.number}`:"—");
                    const assists=r.assists.map(a=>a.players?.name||(a.players?.number?`#${a.players.number}`:"—")).join(", ");
                    return (
  ...
  return (
    <div className="card">
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <tbody>
          {lineup.length===0 && <tr><td colSpan={3} style={{padding:10,color:"#999"}}>—</td></tr>}
          {lineup.map(p=>(
