...
/** team skater stats from game_stats */
function useTeamStats(teamId) {
  const [stats, setStats] = React.useState(new Map()); // player_id -> {gp,g,a,pts}
  React.useEffect(() => {
    let cancelled = false;
    (async () => {
      const { data, error } = await supabase
      ...
      for (const r of data || []) {
        const p = r.player_id;
        if (!byPlayer.has(p)) byPlayer.set(p, { g: 0, a: 0, games: new Set() });
        const agg = byPlayer.get(p);
        agg.g += r.goals || 0;
        agg.a += r.assists || 0;
        if (r.game_id != null) agg.games.add(r.game_id);
      }
      const m = new Map();
      for (const [pid, agg] of byPlayer) {
        m.set(pid, { gp: agg.games.size, g: agg.g, a: agg.a, pts: agg.g + agg.a });
      }
...
function useResizableColumns(teamId, defaults) {
  ...
  const startResize = (col, startX) => {
    ...
    const onMove = (e) => {
      ...
      setWidths((w) => ({ ...w, [col]: Math.max(RESIZE_MIN, startW + delta) }));
    };
    const onUp = () => {
...
export default function TeamPage() {
  ...
  const statsMap = useTeamStats(id);
  ...
  // build combined rows (roster row + stats)
  ...
  // simple sorting (default by pts desc)
  ...
  const Th = ({ col, label, onSort }) => (
    <div
      className="td th-resizable"
      style={{ width: widths[col], minWidth: widths[col], maxWidth: widths[col] }}
      onDoubleClick={() => onSort?.()}
      title="Double-click to sort, drag edge to resize"
    >
      <div className="th-label">
        {label}{" "}
        {sortKey === col ? <span className="muted">{sortDir === "asc" ? "▲" : "▼"}</span> : null}
      </div>
      <span
    ...
    <div>
      ...
      <div className="tbl">
        <div className="tr thead">
          <Th col="player" label="Player" onSort={() => clickSort("name")} />
          <Th col="number" label="#" onSort={() => clickSort("number")} />
          <Th col="pos" label="POS" onSort={() => clickSort("position")} />
          <Th col="gp" label="GP" onSort={() => clickSort("gp")} />
          <Th col="g" label="G" onSort={() => clickSort("g")} />
          <Th col="a" label="A" onSort={() => clickSort("a")} />
          <Th col="pts" label="PTS" onSort={() => clickSort("pts")} />
        ...
        {sortedRows.map((r) =>
          r.__edit ? (
            <div className="tr" key={r.id}>
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setPlayers((cur) =>
                      cur.map((x) =>
                        x.id === r.id ? { ...x, __edit: { ...x.__edit, name: e.target.value } } : x
                      )
                    )
                  }
                />
              ...
              <div
                className="td c"
                style={{ width: widths.number, minWidth: widths.number, maxWidth: widths.number }}
              >
                <input
                  className="in"
              ...
              <div
                className="td c"
                style={{ width: widths.pos, minWidth: widths.pos, maxWidth: widths.pos }}
              >
                <select
                  className="in"
              ...
              {/* Stats cells (read only during edit) */}
              <div className="td c" style={{ width: widths.gp, minWidth: widths.gp, maxWidth: widths.gp }}>
                {r.gp}
              </div>
              <div className="td c" style={{ width: widths.g, minWidth: widths.g, maxWidth: widths.g }}>
                {r.g}
              </div>
              <div className="td c" style={{ width: widths.a, minWidth: widths.a, maxWidth: widths.a }}>
                {r.a}
              </div>
              <div className="td c b" style={{ width: widths.pts, minWidth: widths.pts, maxWidth: widths.pts }}>
                {r.pts}
              </div>
              <div
                className="td right"
