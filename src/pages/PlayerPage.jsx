...
export default function PlayerPage() {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      /* ---------- PLAYER (no number, no team anymore) ---------- */
      ...
      /* ---------- SEASON STATS (leaders_current) ---------- */
      const { data: statRows } = await supabase
        .from("leaders_current")
        .select(
          "season_id, category_id, team, gp, g, a, pts"
        )
        .eq("player_id", pid)
      ...
      /* ---------- HEADER TEAM + NUMBER (latest season/category) ---------- */
      let teamRow = null;
      let jersey = null;
      ...
      if (stats.length > 0) {
        ...
        const { data: tpRow } = await supabase
          .from("team_players")
          .select(
            "number, team_id, teams ( id, name, short_name, logo_url )"
          )
          .eq("player_id", pid)
