...
export default function LivePage() {
  ...
  // realtime refresh for game + events
  useEffect(() => {
    const ch = supabase
      .channel(`rt-live-${slug}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "events" }, () => game && refreshEvents(game.id))
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "games" }, async () => setGame(await getGameBySlug(slug)))
      .subscribe();
  ...
  return (
    <div className="container">
      ...
      {/* 👉 Your existing live controls go here (clock/period/shot & goal buttons/add event). */}

      {/* Events list (same as boxscore), no roster here */}
